...
class ScheduleProvider extends ChangeNotifier {
  ...
  void getSchedules({
    ...
    cache.update(date, (value) => resp, ifAbsent: () => resp);  // 1. 선택한 날짜의 일정들 업데이트하기

    notifyListeners();  // 2. Listening 하는 위젯들 업데이트하기
  }
  ...
  void createSchedule({
    ...
    try {
      ...
      cache.update(  // 1. 서버 응답 기반으로 캐시 업데이트
        targetDate,
            (value) => value
            .map((e) => e.id == tempId
            ? e.copyWith(
  ...
  void deleteSchedule({
    ...
    try {
      await repository.deleteSchedule(id: id); // 1. 삭제함수 실행
    } catch (e) {
      cache.update(
        // 2. 삭제 실패시 캐시 롤백하기
        date,
